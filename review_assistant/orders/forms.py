...
class OrderForms(forms.Form):
    ...
    date_order_end = forms.DateField(label="До какой даты",
                                     widget=forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}))
    amount = forms.CharField(label="Кол-во",
    ...
    link_to_account = forms.URLField(label="Ссылка на обявление",
                                     widget=forms.TextInput(attrs={'class': 'form-control'}))
    text = forms.CharField(label="Текста отзывов", widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 5}))
...
class PhotoForms(forms.Form):
    photo = forms.ImageField(label="Фото обявлений",
